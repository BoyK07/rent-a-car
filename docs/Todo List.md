...
* \[VEREIST\] Maak een GitHub-repository aan voor code en documentatie.
* \[VEREIST\] Stel een logische mapstructuur op (backend / android / docs / media).
* \[VEREIST\] Wijs teamrollen toe (projectleider, backend, frontend, documentatie, testing).
* \[VEREIST\] Stel een globale planning op met deadlines per onderdeel.
* \[VEREIST\] Richt communicatiekanalen in (Teams, Discord, etc.).
* \[VEREIST\] Voeg een .gitignore, README en LICENSE toe in de repository.
...
* \[VEREIST\] Beschrijf de belangrijkste use cases:
  * Auto aanbieden en beheren
  * Auto zoeken en filteren
  * Auto reserveren
  * Route tonen naar huurauto
  * Rijgedrag meten en bonuspunten berekenen
* \[VEREIST\] Schrijf user stories voor alle use cases.
* \[VEREIST\] Stel functionele en niet-functionele eisen vast.
* \[VEREIST\] Definieer gebruikersrollen (eigenaar, huurder).
...
* \[VEREIST\] Gebruik PostgreSQL voor productie, H2 voor lokale tests.
...
* \[VEREIST\] Voeg dependencies toe: Exposed, PostgreSQL-driver, Ktor, Kotlinx Serialization.
* \[VEREIST\] Maak packages aan: routes, models, controllers, services, database.
* \[VEREIST\] Implementeer databaseconfiguratie (PostgreSQL + H2 fallback).
